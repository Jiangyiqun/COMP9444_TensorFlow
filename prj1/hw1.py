...
def my_perceptron(x):
    ...
    w = tf.get_variable("w", dtype=tf.float32,\
            initializer= tf.ones([x]))
    product = tf.multiply(i, w)
    sum_up = tf.reduce_sum(product)
    out = my_relu(sum_up)
    return i, out
...
def onelayer(X, Y, layersize=10):
    ...
    batch_xentropy = -1 * tf.reduce_sum(Y * tf.log(preds), reduction_indices=1)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    x_square = tf.reshape(X, [-1, 28, 28, 1])   # 784 -> 28*28*1
    # conv_layer1
    conv1 = tf.layers.conv2d(x_square, filters=convlayer_sizes[0],\
